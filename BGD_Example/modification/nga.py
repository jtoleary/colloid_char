...
class Snapshot:
    ...
    """
    def __init__(self,xyz=None,box=None,nl=None,pbc='xyz',types=None,total_na=None,tag=None):
    ...
    def parseOptions(self,options):
        ...
        if 'ntotal_na' in options:
            self.n_shells = options['total_na']
...
class Ensemble:
        ...
        for other in others:
            ...
            for key, val in other.graph_lookups.items():
                if key in self.graph_lookups:
                    warnings.warn('Duplicate graph lookup key detected during Ensemble.collect')
                self.graph_lookups[key] = val
    def set_current_landmarks(self):
        R""" sets all current signatures as landmarks (more can be added later)
        """
        if not self.master:
            return
        self.lm_idx = np.arange(self.library.counts.shape[0])
    ...
    def detectDistOutliers(self,mode=None,thresh=None,touch_landmarks=True):
        ...
        if self.master:
            ...
            if not touch_landmarks:
                # We want to keep the landmark indices in both the rows and columns
                self.valid_rows = np.union1d(self.valid_rows, self.lm_idx)
                self.invalid_rows = np.setdiff1d(self.invalid_rows, self.lm_idx)
        ...
        if self.master:
            if self.dists is None:
                if self.lm_idx is None:
                    warnings.warn('Computing pairwise distance matrix between all signatures')
                self.computeDists()
